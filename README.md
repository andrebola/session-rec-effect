...
This is the code accompanying the RecSys submission "Exploring Longitudinal Effects of Session-based Recommendations".

The code is based on the session-based framework by Ludewig et al. (https://github.com/noemi-mauro/session-rec). 

After downloading the datasets, to run the simulations run: `python run_config.py conf/example_simulation.yml`.

In the cofiguration file you can indicate:
  1) the dataset to use in the 'data' section 
  2) the algorithm to use in the 'algorithms' section
  3) in 'simulation' section you can enable 'rerank' (True|False) and specify the kind ('user'|'popular')

In the file `conf/example_simulation.yml` you can find an example for the nowplaying dataset and we also provide
another example of configuration file for 30music dataset in `conf/example_simulation_30music.yml`.
